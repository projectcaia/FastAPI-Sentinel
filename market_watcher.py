# market_watcher.py — FGPT Sentinel 시장감시 워커 (정규장 수정 버전)
# -*- coding: utf-8 -*-
...
def _extract_change_percent(q: dict, symbol: str = None):
    """정규장 데이터 우선, 없으면 일반 데이터 사용"""
    
    # 1. API가 제공하는 변화율 직접 사용 (가장 정확)
    cp = q.get("regularMarketChangePercent")
    if cp is not None:
        return float(cp)
    
    # 2. 한국 시장 특별 처리 (한국 ETF는 regularMarket 필드가 불안정)
    is_kr = symbol and (".KS" in symbol or "^KS" in symbol)
    
    if is_kr:
        # 한국 시장은 price/previousClose 우선 사용
        price = q.get("price") or q.get("regularMarketPrice")
        prev = q.get("previousClose") or q.get("regularMarketPreviousClose")
    else:
        # 미국/기타 시장은 regularMarket 우선
    ...
    # 4. 마지막 시도: fiftyTwoWeekRange 같은 대체 필드는 사용하지 않음
    return None
        ...
        try:
            if provider == "yahoo":
                ...
                for idx, sym in enumerate(symbols):
                    if idx < len(items):
                        cp = _extract_change_percent(items[idx], sym)
                        if cp is not None:
...
def check_and_alert():
    ...
    if sess == "KR":
        ...
        if not us_market_open:
            ...
            symbols = [
                ("ΔES", SYMBOL_SPX_FUT, "ES"),
                ("ΔNQ", SYMBOL_NDX_FUT, "NQ"),
            ]
        else:
