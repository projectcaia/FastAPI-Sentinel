...
class K200FuturesPoller:
    ...
    async def get_current_price(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if http_method == "GET" and require_hashkey:
                logger.debug(
                    "[DBSEC] Hashkey disabled automatically for GET futures quote"
                )
                require_hashkey = False
            hashkey_path = os.getenv("DB_HASHKEY_PATH", "/dfutureoption/hashkey").strip()
