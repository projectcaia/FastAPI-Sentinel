    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        while attempts < max_retries:
            ...
            if simulate_429 and attempts <= simulate_failures:
                ...
                status = 429
                err = {"description": "Too Many Requests (simulated)"}
            else:
                try:
                    r = await client.post(url, data=params)
                    status = r.status_code
                    if status == 200:
                        return {"ok": True, "attempts": attempts, "status": status, "data": r.json()}
                    else:
                        err = r.json() if r.headers.get("content-type","").startswith("application/json") else {"description": r.text}
