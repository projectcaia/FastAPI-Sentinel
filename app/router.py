...
async def jobs(hours: int = Query(24, ge=1, le=168), limit: int = Query(50, ge=1, le=500)):
    con = db.connect(); items = db.recent_jobs(con, hours=hours, limit=limit)
...
async def ingest(request: Request):
    ...
    except Exception: raise HTTPException(status_code=422, detail="invalid json")
    try: model = IngestModel.model_validate(data)
    except Exception as e: raise HTTPException(status_code=422, detail=f"schema error: {e}")
    idem = request.headers.get("Idempotency-Key") or model.idempotency_key
    con = db.connect(); db.migrate(con)
    rec = {"idempotency_key": idem, "source": model.source, "type": model.type, "priority": model.priority,
           "timestamp": model.timestamp, "payload_json": data.get("payload"), "status": "queued"}
    ...
    dispatched = bool(result.get("ok")); status = "pushed" if dispatched else "failed"
    ...
    return {"ok": dispatched, "job_id": job_id, "ack": ack, "dedup": False, "status": status,
            "queued": True, "dispatched": dispatched, "summary_sent": dispatched}
